...
Invoke when:
- The QA Lead is in Mode 1 (Test Planning) and has an approved PRD at `~/.ai-software-agency/projects/<slug>/prd.md`
...
1. **Project slug** (required) — the project directory name under `~/.ai-software-agency/projects/`
...
1. Read the approved PRD:
   ```
   ~/.ai-software-agency/projects/<slug>/prd.md
   ```
   If the file does not exist, stop and report: "No PRD found at `~/.ai-software-agency/projects/<slug>/prd.md`. The Product Manager must create and get it approved first."
...
1. Read `~/.ai-software-agency/projects/<slug>/tests.md` if it exists.
...
The content above will be appended to `~/.ai-software-agency/projects/<slug>/tests.md`.
...
1. Read `~/.ai-software-agency/projects/<slug>/tests.md` one more time to confirm the insertion point is still valid.
...
4. Append to `~/.ai-software-agency/audit.log`:
   ```
   [<ISO-date>] [qa-lead] TEST_CASES_GENERATED: <feature-name> — appended to ~/.ai-software-agency/projects/<slug>/tests.md
   ```
