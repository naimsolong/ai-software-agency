...
description: Present a deliverable or plan to the user for explicit approval before proceeding. This is a mandatory blocking gate — no work advances to the next department without passing through here.
---
...
**APPROVED:**
1. Log approval to `~/.ai-software-agency/audit.log`:
```
...
```
2. Update task status to `done` in `~/.ai-software-agency/tasks.md`
3. Report to CEO: "Gate passed. [Deliverable] approved. Ready for next stage."

**REJECTED:**
1. Log rejection to `~/.ai-software-agency/audit.log`:
```
